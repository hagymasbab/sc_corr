...
for m in range(len(means)):
    ...
    for t in range(len(trialNums)):
        ...
        pl.ylim([0, 5])
        pl.plot(corr * np.ones((1, 2)).T, [0, pl.gca().get_ylim()[1]], color='red', linestyle='-', linewidth=4)
        pl.plot(sc_corr[0, 1] * np.ones((1, 2)).T, [0, pl.gca().get_ylim()[1]], color='green', linestyle='-', linewidth=4)
        pl.xlim([-1, 1])
        pl.set_xticklabels([])
        pl.set_yticklabels([])
