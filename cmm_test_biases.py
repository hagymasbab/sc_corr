...
n_samp = np.floor(n_step_per_chain * n_chain / 2)
...
if recalc:
    ...
    pdfs = np.zeros((n_means, n_corrs, n_reest, n_res))
    ...
    for m in range(n_means):
        for c in range(n_corrs):
            ...
            for i in range(n_reest):
                ...
                if numeric:
                    print("Mean %d/%d, corr %d/%d, reest %d/%d" % (n_means, m+1, n_corrs, c+1, n_reest, i+1))
                    post = cmm.pairwise_corr_numerical_posterior(n_trial, sc_corr[0, 1], n_samp_gen, n_samp.astype(np.int64), n_res)
                    pdfs[m, c, i, :] = post
                    MAP_corr = ct.pdf_map(x, post)
                else:
                    rnd.seed()
                    ...
                    MAP_corr = ct.histogramMode(mps_corr[:, 0, 1], 20)
                est_vs_true[m, c, i, :] = [corr_vals[c], MAP_corr, sc_corr[0, 1], sc_corr_noiseless[0, 1]]

    pickle.dump((est_vs_true, samples, pdfs), open('biastest.pkl', 'wb'))
else:
    ...
    for c in range(n_corrs):
        ...
        for m in range(n_means):
            ...
            for r in range(n_reest):
                ...
                if numeric:
                    act_mean = ct.pdf_mean(x, pdfs[m, c, r_idx, :])
                    act_map = ct.pdf_map(x, pdfs[m, c, r_idx, :])
                    act_std = ct.pdf_std(x, pdfs[m, c, r_idx, :])
                else:
                    act_mean = np.mean(samples[m, c, r_idx, :])
                    act_map = ct.histogramMode(samples[m, c, r_idx, :], 30)
